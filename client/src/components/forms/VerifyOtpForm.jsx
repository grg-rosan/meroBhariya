...
export default function VerifyOtpForm({ email, onVerify, onResend }) {
  ...
  }, [otp, isComplete, loading, onVerify]);

  // Auto-submit when all digits filled
  const hasAutoSubmitted = useRef(false);
  useEffect(() => {
    if (isComplete && !loading && !error && !hasAutoSubmitted.current) {
      hasAutoSubmitted.current = true;
      handleVerify();
    }
  }, [isComplete, loading, error, handleVerify]);
  ...
  const target = email || "your contact";
  ...
  return (
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        {/* Header */}
        <div className="mb-10">
          <div className="w-12 h-12 rounded-2xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center mb-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
            ...
            <span className="text-zinc-700 dark:text-zinc-300 font-medium">{target}</span>.
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-sm">
          {verified ? (
            <div className="text-center py-4">
              ...
              <p className="text-zinc-900 dark:text-white font-medium">Verified!</p>
            ...
            <>
              {/* OTP boxes */}
              <div className="mb-6">
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-4 tracking-wide uppercase">
                  Verification code
                </label>
                  ...
                  {digits.map((d, i) => (
                    <input
                      ...
                      className={`
                        ...
                        bg-zinc-100 dark:bg-zinc-800 border rounded-xl
                        text-zinc-900 dark:text-white
                        outline-none transition-all duration-150 caret-transparent
                        focus:ring-2 focus:ring-offset-0 focus:scale-105
                        ${error
                          ? "border-red-500/60 focus:ring-red-500/20 text-red-500 dark:text-red-400"
                          : d
                            ? "border-zinc-400 dark:border-zinc-500 focus:border-zinc-500 dark:focus:border-zinc-400 focus:ring-zinc-400/20"
              ...
              <button
                ...
                className={`
                  ...
                  ${isComplete && !loading
                    ? "bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-100 active:scale-[0.98]"
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-500">
                  ...
                  {resendCooldown > 0 ? (
                    ...
                    <button
                      ...
                      className="text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors underline underline-offset-2"
                    >
                      Resend code
                    </button>
                  )}
