...
import { MapPin, Camera, CheckCircle, XCircle, Loader2, Navigation2, AlertTriangle } from 'lucide-react';
  ...
  const request = () => new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        setLoc(coords); setL(false); resolve(coords);
      },
      e => { setErr(e.message); setL(false); reject(e); },
...
export default function DeliverPackage() {
  ...
  const handleDeliver = async () => {
    setSub(true); setResult(null); setApiErr(null);

    let coords;
    try {
      coords = await geo.request();
    } catch {
      setApiErr('Could not get GPS location. Enable location in your browser settings.');
      setSub(false); return;
    }
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-500 mt-0.5">Your GPS will be verified before delivery is logged</p>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4 mb-4">
        <div>
          ...
          <input type="number" min="0" value={codCollected} onChange={e => setCod(e.target.value)}
            placeholder="0"
        ...
        <div>
          ...
          <label className="flex items-center gap-3 px-4 py-3 border border-dashed border-zinc-700 hover:border-zinc-500 rounded-xl cursor-pointer transition-all">
            ...
            <input type="file" accept="image/*" capture="environment" className="hidden" onChange={e => setPod(e.target.files[0])} />
          </label>
        </div>
      ...
      {result === 'geofence' && apiError && (
        <div className="p-4 bg-red-500/10 border border-red-700/50 rounded-xl mb-4">
          ...
          <p className="text-xs text-zinc-400 mb-1">You are <span className="text-red-400 font-semibold">{apiError.distanceMeters}m</span> away. Must be within {GEOFENCE_RADIUS}m.</p>
      ...
      {result === 'error' && (
        <div className="flex items-center gap-2 p-4 bg-red-500/10 border border-red-700/50 rounded-xl mb-4">
          ...
          <p className="text-sm text-red-300">{apiError ?? 'Delivery failed. Try again.'}</p>
        </div>
      )}
