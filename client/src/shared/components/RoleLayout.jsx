    ...
    <div className="flex h-screen bg-zinc-950 font-['DM_Sans',sans-serif] overflow-hidden">
      <aside className="w-56 flex flex-col bg-zinc-900 border-r border-zinc-800 shrink-0">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${
                  isActive ? `${accentClass.replace('bg-', 'bg-').replace('500','500/10')} text-white font-medium` : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800'
                }`}
            >
              <Icon size={15} />{label}
            </NavLink>
        ...
        <div className="p-3 border-t border-zinc-800">
          ...
          <button onClick={logout} className="flex items-center gap-2 w-full px-3 py-1.5 text-xs text-zinc-500 hover:text-red-400 rounded-lg hover:bg-zinc-800 transition-all">
