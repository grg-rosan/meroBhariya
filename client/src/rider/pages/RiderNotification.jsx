          ...
          {notifications.map(n => (
            <div key={n.id}
              ...
              <div className="flex items-start gap-3">
                <span className="text-xl">{TYPE_ICON[n.type] ?? TYPE_ICON.default}</span>
                <div className="flex-1">
                  ...
                  <p className="text-xs text-zinc-600 mt-1">{new Date(n.id).toLocaleString()}</p>
                </div>
                {!n.read && <div className="w-2 h-2 rounded-full bg-sky-500 mt-1 shrink-0" />}
              </div>
            </div>
          ))}
        </div>
