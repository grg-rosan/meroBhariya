...
export default function RiderDashboard() {
  ...
  useSocket(user?.id, (event, socketData) => { // ← renamed data→socketData to avoid shadowing
    if (event === 'verified') {
      const n = { type: 'verified', title: 'Account verified!', message: 'You can now go on duty.' };
      push(n);
      setBanner(n);
      refetch();
    ...
    if (event === 'doc_approved') {
      push({ type: 'doc_approved', title: 'Document approved', message: `Your ${socketData?.docType} was approved.` });
    }
    if (event === 'doc_rejected') {
      push({ type: 'doc_rejected', title: 'Document rejected', message: `Your ${socketData?.docType} was rejected. ${socketData?.note ?? ''}` });
    }
  });

  // ← use isVerified from both sources
  const isVerified = data?.rider?.isVerified ?? user?.riderProfile?.isVerified ?? false;
  ...
  const handleToggle = async () => {
    ...
    catch (_) { setOnlineOverride(!next); }
  };

  if (loading) return <div className="p-6 text-zinc-400">Loading...</div>;
  if (error)   return <div className="p-6 text-red-400">Failed to load dashboard.</div>;
    ...
    <div className="p-6 max-w-4xl mx-auto">
      {banner && (
        <div className="mb-4 p-4 bg-green-500/10 border border-green-700 rounded-xl text-green-400 text-sm flex items-center justify-between">
          ...
          <button onClick={() => setBanner(null)} className="text-green-600 hover:text-green-400 ml-4">✕</button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* ← use isVerified not rider.isVerified */}
        <button onClick={handleToggle} disabled={tl || !isVerified}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard icon={Package}    label="Deliveries today" value={stats.deliveriesToday ?? 0}                          color="sky"   />
        <StatCard icon={Banknote}   label="COD collected"    value={`रु ${(stats.codCollected ?? 0).toLocaleString()}`}  color="green" />
        <StatCard icon={MapPin}     label="Km covered"       value={stats.kmCovered ? `${stats.kmCovered} km` : '—'}     color="blue"  />
        <StatCard icon={TrendingUp} label="Today's earnings" value={`रु ${(stats.todayEarnings ?? 0).toLocaleString()}`} color="amber" />
      </div>
          ...
          <div className="relative pl-5">
            {activity.map((item, i) => (
              <div key={item.id} className="relative pb-4 last:pb-0">
                ...
                {i < activity.length - 1 && <div className="absolute -left-[15px] top-3 w-px h-full bg-zinc-800" />}
                <p className="text-sm text-zinc-300">{item.status} — {item.receiverName} · {item.deliveryAddress}</p>
                <p className="text-xs text-zinc-600 mt-0.5">{new Date(item.time).toLocaleTimeString()}</p>
              </div>
            ))}
          </div>
        )}
