...
const DOCS = [
  { key: "citizenshipFront",    label: "Citizenship — Front",             hint: "JPG, PNG or PDF · max 5 MB", accept: "image/*,application/pdf", emoji: "🪪", required: true },
  { key: "citizenshipBack",     label: "Citizenship — Back",              hint: "JPG, PNG or PDF · max 5 MB", accept: "image/*,application/pdf", emoji: "🪪", required: true },
  { key: "licensePhoto",        label: "Driving License",                 hint: "JPG or PNG · max 5 MB",      accept: "image/*",                 emoji: "🪪", required: true },
  { key: "vehicleRegistration", label: "Vehicle Registration (Bluebook)", hint: "JPG, PNG or PDF · max 5 MB", accept: "image/*,application/pdf", emoji: "📋", required: true },
  { key: "profilePhoto",        label: "Profile photo",                   hint: "Clear face photo · JPG/PNG",  accept: "image/*",                 emoji: "🤳", required: true },
];
    ...
    return (
      <PageShell>
        <Card>
          ...
          {hasRejected && (
            <Button onClick={() => { setReupload(true); setFiles({}); setStep(0); }}>
            ...
            <div className="flex flex-col gap-2.5 mb-6">
              {DOCS.map(doc => (
                <div key={doc.key}
                  className="flex items-center justify-between bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3">
                  <div>
                    <p className="text-sm font-medium text-white">{doc.emoji} {doc.label}</p>
          ...
          <>
            ...
            <div className="flex flex-col gap-2 mb-6">
              {DOCS.map(doc => (
                <div key={doc.key}
                  className="flex items-center justify-between bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3">
                  <span className="text-sm text-zinc-400">{doc.emoji} {doc.label}</span>
