...
export function useToggleDuty() {
  ...
  const toggle = async (isOnline) => {
    setLoading(true);
    try { return await apiPatch('/api/rider/status', { isOnline }); }
...
export function useScanPackage() {
  ...
  const scan = async (trackingNumber, action) => {
    ...
    try {
      const data = await apiPost(`/api/shipments/${trackingNumber}/scan`, { action });
...
export function useConfirmDelivery() {
  ...
  const deliver = async (shipmentId, payload) => {
    ...
    try { return await apiPost(`/api/shipments/${shipmentId}/deliver`, payload); }
    catch (e) { setError(e.message); throw e; }
    finally { setLoading(false); }
  };
