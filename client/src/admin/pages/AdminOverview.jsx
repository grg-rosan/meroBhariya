  ...
  return (
    <div className="mb-3 last:mb-0">
      ...
      <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-700 ${color}`} style={{ width: `${safePct}%` }} />
      </div>
    </div>
  );
}
...
const QUICK_ACTIONS = [
  ...
  { label: 'Manage staff',             key: null,               to: '/admin/staff',       color: 'text-zinc-400'  },
];
...
export default function AdminOverview() {
  ...
  const codDisplay = stats?.codHeld != null
    ? `रु ${(stats.codHeld / 100000).toFixed(1)}L`
    : '—';
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-500 mt-0.5">meroBhariya logistics — live health dashboard</p>
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard icon={Users}   label="Active merchants" value={statsLoading ? '…' : (stats?.activeMerchants ?? '—')} color="violet" />
        <StatCard icon={Bike}    label="Active riders"    value={statsLoading ? '…' : (stats?.activeRiders    ?? '—')} color="sky"    />
        <StatCard icon={Package} label="Shipments today"  value={statsLoading ? '…' : (stats?.shipmentsToday  ?? '—')} color="blue"   />
        <StatCard icon={Wallet}  label="COD held"         value={statsLoading ? '…' : codDisplay}                       color="amber"  />
      </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          {healthLoading || qaLoading ? (
            <div className="space-y-4">
              {[...Array(2)].map((_, i) => <div key={i} className="h-5 bg-zinc-800 rounded animate-pulse" />)}
            </div>
          ) : (
            <>
              <Bar label="Delivery success rate" pct={health?.successRate}       color="bg-green-500"  />
              <Bar label="Rider availability"    pct={health?.riderAvailability} color="bg-violet-500" />
            </>
          )}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="space-y-3">
            {actLoading ? (
              ...
              activity.map((a, i) => (
                <div key={i} className="flex gap-3">
                  <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${DOT[a.type] ?? 'bg-zinc-600'}`} />
                  <div>
