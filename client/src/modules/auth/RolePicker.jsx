  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3">
        {ROLES.map((r) => (
          <button
            ...
            className="bg-zinc-950 border border-zinc-200 dark:border-zinc-800 hover:border-orange-500 rounded-xl p-4 text-left transition-colors group"
          >
            <p className="text-sm font-semibold text-white group-hover:text-orange-400 transition-colors">
              {r.emoji} {r.label}
            </p>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{r.desc}</p>
          </button>
        ))}
      </div>
      ...
      <Link to="/login" className="no-underline">
        <button className="w-full bg-transparent border border-zinc-200 dark:border-zinc-800 hover:border-gray-400 text-zinc-400 hover:text-zinc-700 dark:text-zinc-300 text-sm rounded-xl py-2.5 transition-colors">
          Sign in instead
        </button>
      </Link>
    </>
