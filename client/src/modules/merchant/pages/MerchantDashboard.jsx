...
export default function MerchantDashboard() {
  ...
  const activeShipments = shipments.filter((s) =>
    ["PENDING", "ASSIGNED", "IN_HUB", "OUT_FOR_DELIVERY"].includes(s.status),
  ).length;
  const delivered = shipments.filter((s) => s.status === "DELIVERED").length;
  const cancelled = shipments.filter((s) => s.status === "CANCELLED").length;
  const codPending = shipments
    .filter((s) => s.codAmount > 0 && s.status !== "DELIVERED")
              ...
              <thead>
                <tr className="border-b border-zinc-800">
                  {["Tracking", "Receiver", "Address", "Status"].map((h) => (
                    <th
                      key={h}
                      className="text-left px-5 py-2.5 text-xs text-zinc-500 font-medium"
                    >
                      {h}
                    </th>
                  ))}
        ...
        <div className="bg-white dark:bg-gray-900 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="mt-5 pt-4 border-t border-zinc-800 grid grid-cols-2 gap-3 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-zinc-500 mt-0.5">Cancelled</div>
            </div>
          </div>
        </div>
      </div>
