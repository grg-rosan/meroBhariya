...
export default function CreateShipment() {
      ...
      try {
        ...
        if (data.pickup && data.delivery && mapRef.current) {
          ...
          upsertMarker("pickup", pickupLngLat, {
            style:
              "width:12px;height:12px;border-radius:50%;background:#10b981;border:2px solid #fff",
            popup:
              '<div style="color:#111;font-size:12px;font-weight:500">Pickup</div>',
          });
  ...
  const validate = () => {
    ...
    if (!form.deliveryLatLng)
      e.deliveryAddress = "Select an address from the dropdown";
    if (!form.weightKg || form.weightKg <= 0)
      e.weightKg = "Enter package weight";
    if (!form.orderValue || form.orderValue <= 0)
          ...
          <div className="bg-white dark:bg-gray-900 border border-zinc-800 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                />
                {errors.weightKg && (
                  <p className="text-xs text-red-400 mt-1">{errors.weightKg}</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5 font-medium">
              ...
              <>
                <div className="space-y-2 mb-4">
                  {[
                    ...
                    {
                      label: "Base fare",
                      value: `रु ${fareData.fareBreakdown.baseFare}`,
                    },
                    {
                      label: "Distance charge",
                      value: `रु ${fareData.fareBreakdown.distanceFare}`,
                    },
                    {
                      label: "Weight charge",
                      value: `रु ${fareData.fareBreakdown.weightFare}`,
                    },
                    ...(fareData.fareBreakdown.fragileCharge > 0
                      ? [
                          {
                            label: "Fragile",
                            value: `रु ${fareData.fareBreakdown.fragileCharge}`,
                          },
                        ]
                      : []),
                    ...(fareData.fareBreakdown.codCharge > 0
                      ? [
                          {
                            label: "COD charge",
                            value: `रु ${fareData.fareBreakdown.codCharge}`,
                          },
                        ]
                      : []),
                    ...(fareData.fareBreakdown.nightSurcharge > 0
                      ? [
                          {
                            label: "Night surcharge",
                            value: `रु ${fareData.fareBreakdown.nightSurcharge}`,
                          },
                        ]
                      : []),
                  ].map((r) => (
