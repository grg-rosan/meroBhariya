...
export default function ScanIn() {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-900 border border-zinc-800 rounded-xl p-6 mb-4">
        {/* Camera view */}
        {mode === "camera" && (
          ...
          {mode === "manual" && (
            <div className="flex gap-2 justify-center">
              <input
                ...
                className="w-48 px-3 py-2 text-sm bg-gray-100 dark:bg-blue-950 border border-zinc-700 rounded-lg text-zinc-200 placeholder-zinc-600 font-mono focus:outline-none focus:border-emerald-500"
              />
              <button
                onClick={() => handleScan()}
      ...
      <div className="bg-white dark:bg-gray-900 border border-zinc-800 rounded-xl overflow-hidden">
        ...
        {log.length === 0 ? (
          ...
          log.map((item, i) => (
            <div
              key={`${item.tracking}-${i}`}
              className="flex items-center gap-4 px-5 py-3 border-b border-zinc-800/50 last:border-none hover:bg-gray-100 dark:bg-blue-950/20 transition-colors"
            >
              <span className="font-mono text-xs text-zinc-400 w-32 shrink-0">
                {item.tracking}
              </span>
