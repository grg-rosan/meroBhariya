...
export default function LiveTrackingMap({ shipment }) {
  ...
  useEffect(() => {
    ...
    upsertMarker("delivery", [lng, lat], {
      style:
        "width:16px;height:16px;border-radius:50%;background:#f43f5e;border:3px solid #fff",
  ...
  useEffect(() => {
    ...
    upsertMarker("rider", [riderLoc.lng, riderLoc.lat], {
      style:
        "width:14px;height:14px;border-radius:50%;background:#3b82f6;border:2px solid #fff;box-shadow:0 0 0 4px rgba(59,130,246,.3)",
    ...
    <div className="bg-white dark:bg-gray-900 border border-zinc-800 rounded-xl overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Radio
            size={13}
            className={`${riderLoc ? "text-green-400" : "text-zinc-500"} ${riderLoc ? "animate-pulse" : ""}`}
          />
