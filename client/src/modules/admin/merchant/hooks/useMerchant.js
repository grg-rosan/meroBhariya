...
export const useMerchantDashboard = () => useAPI('/api/merchant/dashboard');
export const useShipments = (status = '', page = 1) =>
  useAPI(`/api/shipments?${new URLSearchParams({ ...(status && { status }), page, limit: 20 })}`, [status, page]);
export const useCODLedger = () => useAPI('/api/payments/pending');
...
export function useCreateShipment() {
  ...
  const createShipment = async (payload) => {
    ...
    try { return await apiPost('/api/shipments', payload); }
  ...
  const upload = async (file) => {
    ...
    try {
      ...
      const data = await new Promise((res, rej) => {
        xhr.onload  = () => xhr.status < 300 ? res(JSON.parse(xhr.responseText)) : rej(new Error(`HTTP ${xhr.status}`));
