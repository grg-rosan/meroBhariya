...
export default function CreateStaffModal({ onClose, onCreate }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      /* toast handled upstream in useCreateStaff */
    } finally {
      setLoading(false);
    }
  };
  ...
  const isValid =
    form.name && form.email && form.phone && form.password.length >= 8;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="px-5 py-4 space-y-4">
          {/* role selector */}
          <div>
            <label className="block text-xs font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2">
              Role
            </label>
