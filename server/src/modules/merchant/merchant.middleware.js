...
import { computeFare } from "../../utils/others/computeFare.js";
...
export async function requireVerifiedMerchant(req, res, next) {
  try {
    ...
    if (!profile?.isVerified) {
      return next(
        new AppError("Your merchant account is pending verification.", 403),
      );
    }
    next();
  } catch (err) {
...
export async function computeFareMw(req, res, next) {
  try {
    ...
    if (!deliveryLat || !deliveryLng)
      return next(
        new AppError("deliveryLat and deliveryLng are required.", 400),
      );
    if (!fromDistrictId)
      return next(new AppError("fromDistrictId is required.", 400));
    if (!toDistrictId)
      return next(new AppError("toDistrictId is required.", 400));
    ...
    if (lat < 26.3 || lat > 30.5 || lng < 80.0 || lng > 88.2) {
      return next(
        new AppError("Delivery coordinates appear to be outside Nepal.", 400),
      );
    }
    ...
    if (!vehicleType.fareConfig)
      return next(new AppError(`No fare config for: ${vehicleType.name}`, 400));
    if (Number(weight) > vehicleType.maxWeightKg) {
      return next(
        new AppError(
          `Weight ${weight}kg exceeds max ${vehicleType.maxWeightKg}kg for ${vehicleType.name}.`,
          400,
        ),
      );
    }
    ...
    const [locCheck] = await prisma.$queryRaw`
...
`;
    if (!locCheck?.hasLocation) {
      return next(
        new AppError(
          "Merchant pickup location not set. Please update your profile.",
          400,
        ),
      );
    }
    ...
    const fare = computeFare(vehicleType.fareConfig, distanceKm, zone, {
