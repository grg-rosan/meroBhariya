...
function handleError(res, err) {
  ...
  logger.error({ err }, "[Admin/Settlements]");
  return res.status(500).json({ message: "Internal server error." });
}

// GET /api/admin/settlements/riders?page=&limit=
export async function getRiderSettlementsHandler(req, res) {
  try {
    const { page, limit } = req.query;
    return res.json(
      await settlementsService.getRiderSettlements({
...
export async function getRiderSettlementDetailHandler(req, res) {
  try {
    return res.json(
      await settlementsService.getRiderSettlementDetail(req.params.riderId),
    );
  } catch (err) {
    return handleError(res, err);
  }
...
export async function getShipmentLogsHandler(req, res) {
  try {
    return res.json(
      await settlementsService.getShipmentLogs(req.params.shipmentId),
    );
  } catch (err) {
    return handleError(res, err);
  }
