...
A robust logistics and delivery platform that connects merchants with riders (porters). Built with the PERN stack and containerized for scalability, meroBhariya handles everything from real-time geospatial tracking to secure role-based access.
...
```bash
# Build and start all services (db, redis, rabbitmq, backend, frontend)
...
This launches the database, RabbitMQ, Redis, backend, and frontend services together. On first run, the backend automatically runs Prisma migrations and seeds the database.
...
# Stop and also wipe database volumes
...
- Prisma migrations must be run inside the Docker container:
  ```bash
